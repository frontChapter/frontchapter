...
const HomeBanner: React.FC<HomeBannerProps> = ({ banner: bannerData, brands }) => {
  useEffect(() => {
    const ctx2 = gsap.context(() => {
      const banner = document.querySelector(".banner") as HTMLElement | null;
      const bannerBg = document.querySelector(".banner-bg") as HTMLElement | null;
      ...
      if (banner && bannerBg && header) {
        ...
        const position = ((banner.offsetHeight ?? 0) - (bannerBg.offsetHeight ?? 0)) * 0.4;
        parallaxTl
          .fromTo(
            bannerBg,
            {
